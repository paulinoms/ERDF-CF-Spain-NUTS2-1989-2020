...
resumen_areas_periodo <- BD_FE_1989_2020 |>
  group_by(Narea, Area, Periodo) |>
  summarise(Gasto_m_eur00 = round(sum(Gasto_m_eur00, na.rm = TRUE), 2),
            .groups = "drop") |>
  arrange(Narea, Periodo)

# 12.3 Tabla pivotada: CCAA × Año → columnas = áreas
# Las 7 áreas forman las columnas numéricas de la tabla resultante
...
pivot_ccaa_anio <- BD_FE_1989_2020 |>
...
# 12.4 Tabla pivotada en € constantes 2020: CCAA × Año → columnas = áreas
# Misma estructura que pivot_ccaa_anio pero usando Gasto_m_eur2020
pivot_ccaa_anio_2020 <- BD_FE_1989_2020 |>
  group_by(CCAA, Anio, Area) |>
...
# Forzar 2 decimales y renombrar Anio -> Año SOLO en las tablas de exportación.
# Se usa rename("Año" = Anio): la ñ está dentro de una cadena de texto ("..."),
# donde \u00f1 sí es interpretado correctamente por R. Fuera de strings,
# los escapes Unicode NO funcionan como parte de identificadores R.
BD_export          <- forzar_2dec(BD_FE_1989_2020)      |> rename("A\u00f1o" = Anio)
pivot_export       <- forzar_2dec(pivot_ccaa_anio)       |> rename("A\u00f1o" = Anio)
pivot_export_2020  <- forzar_2dec(pivot_ccaa_anio_2020)  |> rename("A\u00f1o" = Anio)
resumen_export     <- forzar_2dec(resumen_areas)
resumen_p_exp      <- forzar_2dec(resumen_areas_periodo)

# A) .rda — objeto R serializado (carga con: load("BD_FE_1989_2020.rda"))
save(BD_FE_1989_2020, file = "ERDF_CF_Spain_NUTS2_1989_2020.rda")
cat("  ERDF_CF_Spain_NUTS2_1989_2020.rda\n")

# B) Pivot €2000 como objeto .rda independiente
save(pivot_ccaa_anio, file = "ERDF_CF_Spain_NUTS2_pivot_EUR2000.rda")
cat("  ERDF_CF_Spain_NUTS2_pivot_EUR2000.rda\n")

# B2) Pivot €2020 como objeto .rda independiente
save(pivot_ccaa_anio_2020, file = "ERDF_CF_Spain_NUTS2_pivot_EUR2020.rda")
cat("  ERDF_CF_Spain_NUTS2_pivot_EUR2020.rda\n")

# C) Excel principal (7 pestañas):
